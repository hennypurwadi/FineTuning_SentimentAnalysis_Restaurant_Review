...
def analyze_csv_file(file, max_length=128, progress=gr.Progress()):
    """
    ...
    if not model_loaded or model is None or tokenizer is None:
        return "❌ Model not loaded. Please load the model first.", None, None, None
    ...
    try:
        ...
        if not review_columns:
            return "❌ No review column found. Please ensure your CSV has a column named 'review' or 'Review'.", None, None, None
        
        review_col = review_columns[0]  # Use first found review column
        reviews = df[review_col].dropna()
        
        if len(reviews) == 0:
            return "❌ No valid reviews found in the file.", None, None, None
...
"""
        ...
        return summary_text, fig, results_df, csv_content
...
with gr.Blocks(css=custom_css, title="Restaurant Review Sentiment Analysis", theme=gr.themes.Soft()) as demo:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1):
            ...
            model_status = gr.Markdown(
                "👆 Click to load the sentiment analysis model",
        ...
        with gr.Column(scale=2):
            ...
            with gr.Tabs():
                ...
                with gr.TabItem("Upload CSV"):
                    gr.Markdown("### Upload and Analyze CSV File")
                    gr.Markdown("*CSV should have a column named 'review' or 'Review'*")
                    ...
                    analyze_file_btn = gr.Button("Analyze Uploaded Reviews", variant="primary")
    ...
    gr.HTML("""
        <div class="description">
            <h3>📖 How to Use</h3>
            <ol>
                <li><strong>Load Model:</strong> Click "Load Model" to initialize the sentiment analysis model</li>
                <li><strong>Choose Input Method:</strong> Select single review, multiple reviews, or file upload</li>
                <li><strong>Enter Text:</strong> Input your restaurant review(s) for analysis</li>
                <li><strong>Analyze:</strong> Click the analyze button to get sentiment predictions</li>
                <li><strong>Review Results:</strong> See sentiment classification, confidence scores, and visualizations</li>
            </ol>
    ...
    analyze_file_btn.click(
        fn=analyze_csv_file,
        inputs=[file_upload, max_length],
        outputs=[file_result, file_chart, file_dataframe, download_results],
        show_progress=True
    )
